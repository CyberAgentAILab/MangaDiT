...
class TrainingCallback(L.Callback):
    ...
    def on_train_batch_end(self, trainer, pl_module, outputs, batch, batch_idx):
        ...
        if self.total_steps % self.sample_interval == 0:
            ...
            self.generate_a_sample(
                ...
                batch["condition_type_1"][0],  # Use the condition type from the current batch
                batch["condition_type_2"][0],  # Use the condition type from the current batch
            )
    ...
    def generate_a_sample(
        ...
        if condition_type_1 == "lineart" and condition_type_2 == "reference":
            # Load lineart and reference images
            lineart_img = Image.open("assets/lineart_19.jpg").resize(
                (condition_size, condition_size)
            ).convert("RGB")
            ref_img = Image.open("assets/frame_14.jpg").resize(
                (condition_size, condition_size)
            ).convert("RGB")
            
            # Convert to numpy arrays
            lineart_np = np.array(lineart_img)
            ref_np = np.array(ref_img)
            print("Input shapes:", lineart_np.shape, ref_np.shape)
            lineart_img = Image.fromarray(lineart_np)
            ref_img = Image.fromarray(ref_np)
        else:
        ...
        condition_1 = Condition(
            condition_type=condition_type_1,
        ...
        condition_2 = Condition(
            condition_type=condition_type_2,
        ...
        res.images[0].save(
            os.path.join(save_path, f"{file_name}_{condition_type_1}_{condition_type_2}.jpg")
        )
